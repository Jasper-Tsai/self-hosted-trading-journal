...
> **Design system for Self-Hosted Trading Journal**  
> Reference this document for all UI/UX design and component development work.
...
- `src/app/globals.css` — Complete token system, background layers, animations
- `src/components/ui/ambient-background.tsx` — Animated blobs + mouse spotlight
- `src/app/layout.tsx` — Dark mode setup, background integration
- `src/components/navigation.tsx` — Frosted header with Linear styling
- `src/components/ui/*.tsx` — All UI components follow this system

**When Creating New Components:**
1. Use design tokens from `globals.css`
2. Apply multi-layer shadows for depth
3. Use `rounded-2xl` for cards, `rounded-lg` for inputs/buttons
4. Add `active:scale-[0.98]` to interactive elements
5. Use gradient text for important headlines
6. Ensure focus states are prominent
